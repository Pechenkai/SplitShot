...
async def random_forfeit(company_id: int | None = None, db: AsyncSession = Depends(get_db)):
...
async def create_company_forfeit(company_id: int, payload: ForfeitCreate, db: AsyncSession = Depends(get_db)):
    ...
    return await ForfeitRepository(db).create_custom(company_id, payload.text)
